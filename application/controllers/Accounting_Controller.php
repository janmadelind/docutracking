...
class Accounting_controller extends CI_Controller {
	...
	#ADD to return
	public function acc_PR_return($data){
		$remarks = $this->input->post('remarks');
        $destination = $this->input->post('destination'); // alamin kung admin office or euser office
        if($destination != 1 OR $destination != 2 OR $destination != 6 OR $destination != 3 OR $destination != 7){
	...
	public function accounting_addPR_remarks1($data){
		$remarks = $this->input->post('remarks');
		$date_added = date('Y-m-d H:i:s T', time());
		...
		if($remarks_result){
			echo "<script type='text/javascript'>alert('Successfully Updated!').delay(500); </script>";
			$this->accounting_notif();
		}
		else{
			echo "<script type='text/javascript'>alert('Successfully Updated!').delay(500); </script>";
			$this->accounting_notif($data);
