...
class Op_controller extends CI_Controller {
	...
	public function op_addPR_remarks1($data){
		$remarks = $this->input->post('remarks');
		$date_added = date('Y-m-d H:i:s T', time());
		$remarks_result=$this->docutrackingmodel->add_PRremarks($data,$remarks,$date_added,$_SESSION['admin_user_ID']);
		if($remarks_result){
	...
	#ADD to return
	public function op_PR_return($data){		
		$remarks = $this->input->post('remarks');
        $destination = $this->input->post('destination'); // alamin kung admin office or euser office
        if($destination != 1 OR $destination != 2 OR $destination != 6){
            $date_added = date('Y-m-d H:i:s T', time());
            if($remarks == NULL){
	...
	public function op_scanPR(){
		...
		if($data['prinfo']['result']== 0){
			...
			else if(($sequence-3) == $_SESSION["admin_office_ID"]){
