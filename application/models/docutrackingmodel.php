...
class docutrackingmodel extends CI_Model {
	...
	public function read_PRdetails($prno){
		$query = $this->db->query (
			"SELECT doc.*, eu.end_user_name, co.college_name, de.department_name, mp.mode_of_procurement, tp.type_name, b.bidders_name
			FROM document doc
	...
	public function update_scannedPR($prno,$scanned_by,$scanned_where){
		...
		$date_end = date('Y-m-d H:i:s T', strtotime($date_scanned.' + 7 days'));
		$query = $this->db->query (
			"UPDATE route_per_scanned
	...
	// INSERT notif 
	public function addNotif($prno,$subj,$officeID,$date){
	...
	public function readNotif($officeID){
		if($officeID!=1 && $officeID!=6){
			$query = $this->db->query (
				"SELECT *
				FROM notification
				WHERE admin_office_ID = '$officeID' AND department_ID = 0
	...
	//READ all NOTIF
	// public function readALLNotifperOffice(){
	// 	$query = $this->db->query (
	// 		"SELECT *
	// 		FROM notification
	// 		WHERE 			
	...
	public function checkNotif($officeID){
		if($officeID!=1 && $officeID!=6){
			$query = $this->db->query (
				...
				AND NOT EXISTS (select * from seen_notif as s where s.notif_ID = n.notif_ID and s.admin_office_ID = $officeID)"
			);
		}
		else{
			$query = $this->db->query (
	...
	public function update_notif(){
		...
		if($admin_office_ID!=1 && $admin_office_ID!=6){
			$query = $this->db->query (
	...
	public function getDuePR($officeID){
		$query = $this->db->query (
			...
			AND status_if_scanned = 'S' AND is_expired != 1
			"
		);
		if($query->num_rows() > 0){
			return $query->result();
